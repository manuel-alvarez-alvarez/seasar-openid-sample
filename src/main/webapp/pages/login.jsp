    ...
    <form name="openid-form-redirection" action="${message.OPEndpoint}" method="post" accept-charset="utf-8">
        <table width="100%" border="0">
            <c:forEach var="parameter" items="${message.parameterMap}">
                <tr>
                    <td style="width: 25%; text-align: right; padding-right: 1em"><label
                            for="${parameter.key}">${parameter.key}</label></td>
                    <td><input type="text" name="${parameter.key}" value="${parameter.value}" style="width: 100%"/>
                    <td>
                </tr>
            </c:forEach>
        </table>
